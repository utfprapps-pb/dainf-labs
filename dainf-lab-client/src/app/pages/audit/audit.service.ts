...
export class AuditService extends BaseService {
  ...
  search(request: AuditSearchRequest): Observable<Page<AuditEntry>> {
    return this._http
      .post<Page<Omit<AuditEntry, 'id'>>>(`${this._url}/search`, request)
      .pipe(
        map((page) => ({
          ...page,
          content: page.content.map((entry) => ({
            ...entry,
            id: entry.revisionId,
          })),
        })),
  ...
  getChanges(
    ...
    entityId: number,
    revisionId: number,
  ): Observable<AuditFieldChange[]> {
    return this._http.get<AuditFieldChange[]>(
      `${this._url}/${entityKey}/${entityId}/${revisionId}/changes`,
    );
  }
}
