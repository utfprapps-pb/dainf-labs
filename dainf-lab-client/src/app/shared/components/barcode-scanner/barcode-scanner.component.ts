...
@Component({
  ...
  template: `
    @if (isAdvancedUser()) {
      <div
        class="flex items-center gap-2 mb-4 p-4 border border-blue-200 dark:border-blue-800/50 bg-blue-50 dark:bg-blue-900/20 rounded-lg"
      >
        <i class="pi pi-barcode text-blue-500 text-2xl"></i>
        <div class="flex-1">
          <label
            class="block text-sm font-semibold text-blue-800 dark:text-blue-300 mb-1"
            >Leitura Rápida (Código de Barras)</label
          >
          <input
            pInputText
            type="text"
          ...
          <small class="text-blue-600 dark:text-blue-400 block mt-1">
            O sistema identificará automaticamente se o código pertence a um
            aluno ou item.
          </small>
        </div>
      </div>
...
export class BarcodeScannerComponent {
  ...
  isAdvancedUser = toSignal(this.userService.hasAdvancedPrivileges(), {
    initialValue: false,
  });
  ...
  onScan() {
    ...
    this.userService
      .search({
        filters: [{ field: 'documento', value: code, type: 'EQUALS' }],
      })
      .subscribe({
        next: (userPage: any) => {
          if (userPage.content && userPage.content.length > 0) {
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Aluno Encontrado',
              detail: `Matrícula: ${user.documento} - ${user.nome}`,
            });
  ...
  private searchItemBySiorg(code: string) {
    this.itemService
      .search({ filters: [{ field: 'siorg', value: code, type: 'EQUALS' }] })
      .subscribe({
        next: (itemPage: any) => {
          if (itemPage.content && itemPage.content.length > 0) {
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Item Encontrado',
              detail: `Siorg: ${item.siorg} - ${item.name}`,
            });
  ...
  private searchItemByAsset(code: string) {
    ...
    this.itemService
      .search({
        filters: [
          { field: 'assets.serialNumber', value: code, type: 'EQUALS' },
        ],
      })
      .subscribe({
        next: (itemPage: any) => {
          if (itemPage.content && itemPage.content.length > 0) {
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Ativo Encontrado',
              detail: `Patrimônio: ${code} - ${item.name}`,
            });
            this.itemScanned.emit(item);
          } else {
            this.messageService.add({
              ...
              detail:
                'O código bipado não corresponde a nenhum aluno ou item cadastrado.',
            });
          }
        },
        error: () => {
          this.messageService.add({
            severity: 'error',
            summary: 'Erro de Busca',
            detail: 'Erro ao pesquisar código.',
          });
        },
      });
  }
